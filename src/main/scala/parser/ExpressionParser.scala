...
object ExpressionParser {
  ...
  val postfixExp : P[Expression] = P(primaryExp | cloneExp | objectCreationExp | postfixIncrementExp | postfixDecrementExp)
  //TODO

  val cloneExp : P[Expression] = P(Fail)
  val postfixIncrementExp : P[Expression] = P(Fail)
  val postfixDecrementExp : P[Expression] = P(Fail)
  val arrayElementInitializer : P[ArrayElementInitializer] = P(Fail)
  val objectCreationExp : P[Expression] = P(Fail)
  //TODO

  //val postfixIncrementExp : P[Expression] = variable <~ "++" ^^ {case va => PostfixIncrementExp(va)}

  //val postfixDecrementExp : P[Expression] = variable <~ "--" ^^ {case va => PostfixIncrementExp(va)}

  //val cloneExp : P[Expression] = "clone" ~> expression ^^ {case exp => CloneExp(exp)}

  val primaryExp : P[Expression] = P(variable | classConstAccExp | constAccExp |
  ...
  val intrinsic : P[Expression] = P(Fail)
