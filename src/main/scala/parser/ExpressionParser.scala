...
object ExpressionParser {
  ...
  val yieldExp : P[Expression] = P((YIELD ~~ ws ~ FROM ~~ space ~/ expression).map(YieldFromExp) |
  ...
  val listAssignment : P[Expression] = P(listIntrinsic ~ "=" ~ assignmentCondExp).map(t => ListAssignmentExp(t._1, t._2))
  ...
  val listIntrinsic : P[ListIntrinsic] = P(LIST ~ "(" ~ ((
    ",".? ~ expression.rep(sep=",".rep(1))).map(Left(_)) |
