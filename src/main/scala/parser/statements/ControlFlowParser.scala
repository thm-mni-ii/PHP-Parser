...
object ControlFlowParser {
  ...
  val ifStmnt: P[IfStmnt] = {
    val ifBody: P[(Seq[Statement], Seq[(Expression, Seq[Statement])], Option[Seq[Statement]], Option[Text])] = P((
      ":" ~/ statements
  ...
  val switchStmnt: P[SwitchStmnt] = {
    ...
    val switchBody: P[(Seq[SwitchBlock], Option[Text])] = P(
  ...
  val whileStmnt: P[WhileStmnt] = {
    val whileBody: P[(Seq[Statement], Option[Text])] = P(
  ...
  val forStmnt: P[ForStmnt] = {
    val forBody: P[(Seq[Statement], Option[Text])] = P(
      (":" ~/ statements ~ ENDFOR ~ semicolonFactory)
        | statement.map(t => (Seq(t), None))
    )

    val forExpressionList: P[ForExpressionList] =
  ...
  val foreachStmnt: P[ForeachStmnt] = {
    val foreachBody: P[(Seq[Statement], Option[Text])] = P(
  ...
  val jumpStmnt: P[JumpStmnt] = P(
    (GOTO ~/ name ~ semicolonFactory).map(t => GotoStmnt(t._1, t._2))
